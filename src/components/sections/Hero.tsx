...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (el) el.style.transform = `translate3d(0, ${y * 0.15}px, 0) scale(1.06)`;
      });
    };
  ...
  return (
    <section id="top" className="relative w-full overflow-hidden bg-[#4d97d4]">
      {/* Full composite scene (parallax on scroll) */}
      <img
        ref={imgRef}
        data-parallax="hero"
        src={HERO_SCENE}
      ...
      {/* Title — vertically centered in the viewport (slightly raised) */}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-[20%] bg-gradient-to-b from-transparent to-white" />
    </section>
  );
}
